...
When a Steam MCP server is available, replace the Shell `curl` calls with MCP tool calls (e.g., `steam.getAppDetails({ appid })`). The data extraction and formatting logic remains the same.
